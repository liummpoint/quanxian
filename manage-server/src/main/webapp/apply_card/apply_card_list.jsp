...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 申请信用卡 <span
        class="c-gray en">&gt;</span> 申请信用卡列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px"
...
<c:if test="${fn:contains(sessionScope.role.websiteRole,sessionScope.user_add)}">
    <div class="cl pd-5 bg-1 bk-gray mt-20"><span class="l"><a
            href="javascript:;" onclick="addInfo()" class="btn btn-primary radius"><i
            class="Hui-iconfont">&#xe600;</i> 申请信用卡</a></span></div>
</c:if>
<c:if test="${fn:contains(sessionScope.role.websiteRole,sessionScope.user_list)}">
    <div class="mt-20">
        <table class="table table-border table-bordered table-hover table-bg table-sort text-c " id="table">
            <thead>
            <tr class="text-c">
                <th id="id" width="80">申请信用卡ID</th>
                <th width="100">用户ID</th>
                <th width="90">订单号</th>
                <th width="150">产品活动ID</th>
                <th width="130">申请者姓名</th>
                <th width="130">申请者身份证号</th>
                <th width="70">申请者手机号码</th>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        var _table = $table.dataTable($.extend(true, {}, CONSTANT.DATA_TABLES.DEFAULT_OPTION, {
            ...
            columns: [
                ...
                {
                    data: "productActivityId",//字段名
                },
                {
                    data: "applyName", //字段名
                },
                {
                    data: "applyIdCardNo", //字段名
                },
                {
                    data: "applyPhone", //字段名
            ...
            "createdRow":
                ...
                function (row, data, index) {
                    ...
                    var update_html='<a style="text-decoration:none;color: cornflowerblue;" onClick="editInfo(' + data.applyCardId+')" >修改</a><span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>'
                    var delete_html='<a style="text-decoration:none;color: cornflowerblue;" onClick="deleteInfo('+ data.applyCardId+')" >删除</a><span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>'

                    $('td', row).eq(10).append(update_html)
    ...
    function editInfo(id) {
        var url = "<%=basePath%>apply_card/apply_card_update.jsp?id=" + id;
        layer_show('修改申请信用卡', url, '', '510');
    }
    function addInfo() {
        var url = "<%=basePath%>apply_card/apply_card_insert.jsp";
        layer_show('新增申请信用卡', url, '', '510');
    }

    function deleteInfo(id) {

        if(confirm("确定要删除吗？")){
            var url = 'applyCard/deleteApplyCard.do';
            $.ajax({
                type: "post",
                url: '<%=basePath%>' + url,
                data: {"applyCardId": id},
