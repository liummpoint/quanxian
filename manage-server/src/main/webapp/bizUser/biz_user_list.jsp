...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 用户 <span
...
<c:if test="${fn:contains(sessionScope.role.websiteRole,sessionScope.user_add)}">
    <div class="cl pd-5 bg-1 bk-gray mt-20"><span class="l"><a
            ...
            class="Hui-iconfont">&#xe600;</i> 用户</a></span></div>
</c:if>
<c:if test="${fn:contains(sessionScope.role.websiteRole,sessionScope.user_list)}">
    <div class="mt-20">
        <table class="table table-border table-bordered table-hover table-bg table-sort text-c " id="table">
            <thead>
            <tr class="text-c">
                <th id="id" width="80">用户昵称</th>
                <th width="100">类型</th>
                <th width="90">手机号</th>
                <th width="150">密码</th>
                <th width="130">用户状态</th>
                <th width="130">父用户ID</th>
                <th width="70">级别</th>
                <th width="100">订单统计</th>
                <th width="100">预结算费用</th>
                <th width="100">已结算费用</th>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        var _table = $table.dataTable($.extend(true, {}, CONSTANT.DATA_TABLES.DEFAULT_OPTION, {
            ...
            columns: [
                ...
                {
                    data: "orderStatistics"//字段名

                },
                {
                    data: "preDistributeFee"//字段名

                },
                {
                    data: "finishDistributeFee"//字段名
            ...
            "createdRow":
                ...
                function (row, data, index) {
                    ...
                    var update_html='<a style="text-decoration:none;color: cornflowerblue;" onClick="editInfo(' + data.bizUserId+')" >修改</a><span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>'
                    var delete_html='<a style="text-decoration:none;color: cornflowerblue;" onClick="deleteInfo('+ data.bizUserId+')" >删除</a><span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>'
    ...
    function editInfo(id) {
        var url = "<%=basePath%>biz_user/biz_user_update.jsp?id=" + id;
        layer_show('修改用户', url, '', '510');
    }
    function addInfo() {
        var url = "<%=basePath%>biz_user/biz_user_insert.jsp";
        layer_show('新增用户', url, '', '510');
    }

    function deleteInfo(id) {

        if(confirm("确定要删除吗？")){
            var url = 'bizUser/deleteBizUser.do';
            $.ajax({
                type: "post",
                url: '<%=basePath%>' + url,
                data: {"bizUserId": id},
