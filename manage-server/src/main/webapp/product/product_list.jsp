...
<c:if test="${fn:contains(sessionScope.role.websiteRole,sessionScope.user_add)}">
    <div class="cl pd-5 bg-1 bk-gray mt-20"><span class="l"><a
            ...
            class="Hui-iconfont">&#xe600;</i> 添加产品</a></span></div>
</c:if>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        var _table = $table.dataTable($.extend(true, {}, CONSTANT.DATA_TABLES.DEFAULT_OPTION, {
            ...
            columns: [
                ...
                {
                    data: "productType",//字段名
                },
                {
                    data: "productName",//字段名
                },
                {
                    data: "pictureUrl",//字段名
                },
            ...
            "createdRow":
                ...
                function (row, data, index) {
                    ...
                    var update_html='<a style="text-decoration:none;color: cornflowerblue;" onClick="editInfo(' + data.productId+')" >修改</a><span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>'
                    var delete_html='<a style="text-decoration:none;color: cornflowerblue;" onClick="deleteInfo('+ data.productId+')" >删除</a><span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>'

                    $('td', row).eq(8).append(update_html)
                    $('td', row).eq(8).append(delete_html)
                },
    ...
    function editInfo(id) {
        var url = "<%=basePath%>product/product_update.jsp?id=" + id;
        layer_show('修改产品', url, '', '510');
    }
    function addInfo() {
        var url = "<%=basePath%>product/product_insert.jsp";
        layer_show('新增产品', url, '', '510');
    }

    function deleteInfo(id) {

        if(confirm("确定要删除吗？")){
            var url = 'product/deleteProduct.do';
            $.ajax({
                type: "post",
                url: '<%=basePath%>' + url,
                data: {"productId": id},
