...
<script type="text/javascript">
    $(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#sessionKey").val(data.data.sessionKey);
                ...
                $("select[name=menuGrade] option").each(function () {
                    if (data.data.menuGrade == $(this).val() ) {
                        $(this).attr('selected', 'selected');
                    }
                })
                parentMenu(data.data.menuGrade,data.data.parentId);
            }
        })
        ...
        $("#form-member-add").validate({
            ...
            submitHandler: function () {
                ...
                $.ajax({
                    ...
                    url: '<%=basePath%>menu/updateMenu.do',
    ...
    function parentMenu(menuGradeValue,parentId) {
        if(menuGradeValue == 1){
            $("#parentMenu_div").hide()
            ...
            $("#parentMenu_div").show()
            $("#parent2Menu_div").hide()
            loadParentMenu(0,"parentMenu",parentId)
            ...
            $.ajax({
                ...
                url: '<%=basePath%>menu/menuOne.do',
                success: function (data) {
                    nowTwoParentId=data.data.parentId;
                    loadParentMenu(nowTwoParentId,"parent2Menu",parentId)//加载2级菜单
                }
            })
            $.ajax({
                type: 'post',
                data: {"id": nowTwoParentId},
                dataType: 'json',
                async: false,
                url: '<%=basePath%>menu/menuOne.do',
                success: function (data) {
                    nowOneParentId=data.data.parentId;
                    loadParentMenu(0,"parentMenu",nowTwoParentId)//加载1级菜单
                }
            })
        }
    }
    function loadParentMenu(parentId,id,nowParentId){
        $.ajax({
            type: 'post',
